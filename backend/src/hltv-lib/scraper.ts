...
const createAttachMethods = ($: cheerio.Root) => {
  const attachMethods = (root: cheerio.Cheerio): HLTVPageElement => {
    const obj: HLTVPageElement = {
      ...
      toArray(): HLTVPageElement[] {
        return root.toArray().map((el) => attachMethods($(el)));
      },
      ...
      filter(
        ...
        return attachMethods(
          root.filter((i, el) => func(i, attachMethods($(el)))),
        );
      },
...
export const HLTVScraper = (root: cheerio.Root): HLTVPage => {
  const attachMethods = createAttachMethods(root);
